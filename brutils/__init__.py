...
data_dir = os.path.join(os.path.dirname(__file__), "..", "..", "dependencies", "data") + "/"
...
def state_dma_lat_lon():
    ...
    s = pd.DataFrame([x.split('\t') for x in s])
    s.columns = s.iloc[0]
    s = s.iloc[1:]
    s['latitude'] = s['latitude'].astype('float32')
    s['longitude'] = s['longitude'].astype('float32')
    return s
